	...
	private ToXContent make(Condition cond, String name, Object value) throws SqlParseException {
		...
		case IN:
		    ...
		    if (cond.getNameExpr() instanceof SQLCaseExpr) {
                /*
                zhongshu-comment 调用CaseWhenParser解�?将Condition的nameExpr属性对象解�?为script query
                �?�考了SqlParser.findSelect()方法，看它是如何解�?select中的case when字段的
                 */
                String scriptCode = new CaseWhenParser((SQLCaseExpr) cond.getNameExpr(), null, null).parseCaseWhenInWhere((Object[]) value);
                /*
                zhongshu-comment
                �?�考DefaultQueryAction.handleScriptField() 将上文得到的scriptCode�?装为es的Script对象，
                但�?��?是完全相�?�，因为DefaultQueryAction.handleScriptField()是处�?�select�?�?�中的case when查询，对应es的script_field查询，
                而此处是处�?�where�?�?�中的case when查询，对应的是es的script query，具体�?看官网文档，�?�索关键字是"script query"

 ...
 *
 * <copyright file="CRFSegment.java" company="上海林原信�?�科技有�?公�?�">
...
public class CRFSegment extends CharacterBasedSegment
{
    ...
    public CRFSegment(String modelPath)
    {
        ...
        logger.info("CRF分�?模型正在加载 " + modelPath);
        long start = System.currentTimeMillis();
        crfModel = CRFModel.loadTxt(modelPath, new CRFSegmentModel(new BinTrie<FeatureFunction>()));
        if (crfModel == null)
        {
            String error = "CRF分�?模型加载 " + modelPath + " 失败，耗时 " + (System.currentTimeMillis() - start) + " ms";
            logger.severe(error);
            throw new IllegalArgumentException(error);
        }
        else
            logger.info("CRF分�?模型加载 " + modelPath + " �?功，耗时 " + (System.currentTimeMillis() - start) + " ms");
    ...
    {
        throw new UnsupportedOperationException("暂�?支�?");

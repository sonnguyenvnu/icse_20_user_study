...
public abstract class BoltSendableResponseCallback<T> implements SendableResponseCallback<T> {
     ...
     */
    protected void init() {
        // 从ThreadLocal�?�出当�?长连接，request等信�?�设置进去，需�?接到请求时�??�?设置到ThreadLocal里
    ...
    /**
     * A->B(当�?)->C的场景下，将远程�?务端C的结果异步返回给调用者A
     *
    ...
    /**
     * A->B(当�?)->C的场景下，将远程�?务端C的业务异常异步返回给调用者A
     *
    ...
    /**
     * A->B(当�?)->C的场景下，将远程�?务端C的RPc异常异步返回给调用者A
     *

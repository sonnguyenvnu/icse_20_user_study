...
public class DataGridTag extends TagSupport {
	...
	protected String searchFields = "";// 查询字段  Author:qiulu  Date:20130618 for：添加对区间查询的支�?
	protected String name;// 表格标示
	protected String title;// 表格标示
	protected String idField="id";// 主键字段
	protected boolean treegrid = false;// 是�?�是树形列表
	protected List<DataGridUrl> urlList = new ArrayList<DataGridUrl>();// 列表�?作显示
	protected List<DataGridUrl> toolBarList = new ArrayList<DataGridUrl>();// 工具�?�列表
	protected List<DataGridColumn> columnList = new ArrayList<DataGridColumn>();// 列表�?作显示
	protected List<ColumnValue> columnValueList = new ArrayList<ColumnValue>();// 值替�?�集�?�
	protected List<ColumnValue> columnStyleList = new ArrayList<ColumnValue>();// 颜色替�?�集�?�
	public Map<String, Object> map;// �?装查询�?�件
	private String actionUrl;// 分页�??交路径
	public int allCount;
	public int curPageNo;
	public int pageSize = 10;
	public boolean pagination = true;// 是�?�显示分页
	private String width;
	private String height;
	private boolean checkbox = false;// 是�?�显示�?选框
	private boolean showPageList = true;// 定义是�?�显示页�?�列表
	private boolean openFirstNode = false;//是�?是展开第一个节点
	private boolean fit = true;// 是�?��?许表格自动缩放，以适应父容器
	private boolean fitColumns = true;// 当为true时，自动展开/�?��?�列的大�?，以适应的宽度，防止横�?�滚动.
	private boolean collapsible = false;// 当为true时，开�?�收起/展开，默认�?�?�动.
	private String sortName;//定义的列进行排�?
	private String sortOrder = "desc";//定义列的排�?顺�?，�?�能是"递增"或"�?�?".
	private boolean showRefresh = true;// 定义是�?�显示刷新按钮
	private boolean showText = true;// 定义是�?�显示刷新按钮
	private String style = "easyui";// 列表样�?easyui,datatables,jqgrid
	private String onLoadSuccess;// 数�?�加载完�?调用方法
	private String onClick;// �?�击事件调用方法
	private String onDblClick;// �?�击事件调用方法
	private String queryMode = "single";//查询模�?
	private String entityName;//对应的实体对象
	private String rowStyler;//rowStyler函数
	private String extendParams;//扩展�?�数,easyui有的,但是jeecg没有的�?�数进行扩展
	private boolean autoLoadData=true; // 列表是�?�自动加载数�?�
	//private boolean frozenColumn=false; // 是�?�是冰冻列    默认�?是
	private String langArg;

	private boolean nowrap = true;

	private Boolean singleSelect;//是�?��?�选true,false

	protected String cssTheme ;

	private boolean isShowSearch=false;//检索区域是�?��?�收缩

	
	private String treeField;//树形列表展示列

	private String btnCls;//列表上方button样�?class属性	

	protected CgFormHeadEntity head;
	protected Map<String, Object> tableData  = new HashMap<String, Object>();
	private String configId = "";
	private boolean isShowSubGrid=false;//是�?�显示表体数�?� 值为true 或者false
	
	private String component;//列表组件�??称（默认easyui,bootstrap-table）

	private boolean query=true;//是�?�显示查询�?�件（默认true,显示为true,�?显示为false）
	...
	private String complexSuperQuery = ""; //根�?�表的编�?是�?�存在展示高级查询构造器
...
//         log.info("=============================== DataGridTag 结�?�时间:"+sdf.format(new Date())+"==============================");
			...
			for (DataGridColumn column : columnList) {
				if(column.isQuery()){
					...
					if(oConvertUtils.isNotEmpty(dictionary)){
						//字典数�?�信�?�，存在两�?处�?�方�?，一�?是表格元素数�?�，一�?是字典表当中的数�?�
...
//		}
	...
	public StringBuffer datatables() {
		...
		sb.append("\"bProcessing\" : true,");// 当datatable获�?�数�?�时候是�?�显示正在处�?��??示信�?�"
		sb.append("\"bPaginate\" : true,"); // 是�?�分页"
		sb.append("\"sPaginationType\" : \"full_numbers\",");// 分页样�?full_numbers,"
		sb.append("\"bFilter\" : true,");// 是�?�使用内置的过滤功能"
		sb.append("\"bSort\" : true, ");// 排�?功能"
		sb.append("\"bAutoWidth\" : true,");// 自动宽度"
		sb.append("\"bLengthChange\" : true,");// 是�?��?许用户自定义�?页显示�?�数"
		sb.append("\"bInfo\" : true,");// 页脚信�?�"
		sb.append("\"sAjaxSource\" : \""+ actionUrl + "&field=" + fields+"\",");
		sb.append("\"bServerSide\" : true,");// 指定从�?务器端获�?�数�?�
		sb.append("\"oLanguage\" : {" + "\"sLengthMenu\" : \" _MENU_ �?�记录\"," + "\"sZeroRecords\" : \"没有检索到数�?�\"," + "\"sInfo\" : \"第 _START_ 至 _END_ �?�数�?� 共 _TOTAL_ �?�\"," + "\"sInfoEmtpy\" : \"没有数�?�\"," + "\"sProcessing\" : \"正在加载数�?�...\"," + "\"sSearch\" : \"�?�索\"," + "\"oPaginate\" : {" + "\"sFirst\" : \"首页\"," + "\"sPrevious\" : \"�?页\", " + "\"sNext\" : \"�?�页\"," + "\"sLast\" : \"尾页\"" + "}" + "},"); // 汉化
		// 获�?�数�?�的处�?�函数 \"data\" : {_dt_json : JSON.stringify(aoData)},
...
//					}
		...
		if (columnList.size() > 0) {
			sb.append("function " + name + "search(){");
			//update by jg_renjie at 2016/1/11 for:TASK #823 增加form实现Form表�?�验�?
			sb.append("try { if(! $(\"#"+name+"Form\").Validform({tiptype:3}).check()){return false;} } catch (e){}");
			sb.append("if(true){");
			//update by jg_renjie at 2016/1/11 for:TASK #823 增加form实现Form表�?�验�?
			...
			//update by jg_renjie at 2016/1/11 for:TASK #823 增加form实现Form表�?�验�?,此处�?��?reset时走验�?，代�?�?�了冗余
		...
		if(hasQueryColum(columnList) && "group".equals(getQueryMode())){
			...
			sb.append("<span><img style=\"margin-top:-3px;vertical-align:middle;\" src=\"plug-in/easyui/themes/icons/ti.png\"  title=\"�??示：模糊查询通�?符: *，多个关键字用�?�角逗�?� ',' 隔开�?\" alt=\"�??示：模糊查询通�?符: *，多个关键字用�?�角逗�?� ',' 隔开�?\" /></span>");
		...
		if(toolBarList.size()>0)
		{
			...
			if(hasMore){
				...
				sb.append("</ul></div>");
				//sb.append("<div class='btn-group'><button data-toggle='dropdown' class='btn btn-default dropdown-toggle'>�?作<span class='caret'></span> </button><ul class='dropdown-menu'><li><a href='buttons.html#'>置顶</a></li><li><a href='buttons.html#' class='font-bold'>修改</a></li><li><a href='buttons.html#'>�?用</a></li><li class='divider'></li> <li><a href='buttons.html#'>删除</a></li> </ul> </div>");
			}

		}
		sb.append("</span>");

		 if("single".equals(getQueryMode())&& hasQueryColum(columnList)){//如果表�?�是�?�查询
		...
		}else if ("advanced".equals(getQueryMode()) && hasQueryColum(columnList)) {// 如果表�?�是高级查询
			sb.append("<span style=\"float:right\">");
			if (btnCls != null && !btnCls.equals("easyui")) {// 自定以样�? bootstrap按钮样�?
		...
		if(superQuery) {
			if(btnCls != null && !btnCls.equals("easyui")) {//自定义bootstrap按钮样�?
	 ...
	 * 仅用于 加载�?更多�?作】 按钮的方法
	 * @param sb 
	 * @param 是�?�是�?更多�?作】按钮，false则为点击�?更多�?作】�?�展示的按钮
	 * @param toolBar
	 */
		...
		if(isShow){
			if (btnCls != null && !btnCls.equals("easyui")) {
				if(btnCls.indexOf("bootstrap")==0){
					...
					sb.append("><i class=\"fa fa-caret-down\"></i><span class=\"bigger-110 no-text-shadow\">更多�?作</span></button>");
				}else{
					sb.append("<a href=\"javascript:void(0)\" onclick='toggleMoreToolbars(this)' class=\""+btnCls+" " + toolBar.getIcon()+"\" >更多�?作</a>");
				}
			}else if(btnCls == null || btnCls.equals("easyui")){
				sb.append("<a href=\"javascript:void(0)\"  onclick='toggleMoreToolbars(this)' class=\"easyui-linkbutton\" plain=\"true\" icon=\"icon-caret-down\">更多�?作</a> ");
	...
	private void loadToolbar(DataGridUrl toolBar,StringBuffer sb){
		...
		}else if(btnCls == null || btnCls.equals("easyui")){//easyUI按钮样�?
	...
	private void getSearchButton(StringBuffer sb) {
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表�?�是组�?�查询
...
//								}
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DataGridUrl dataGridUrl : list) {
			if(!dataGridUrl.isInGroup()){
				...
				String exp = dataGridUrl.getExp();// 判断显示表达�?
				...
				if (OptTypeDirection.Del.equals(dataGridUrl.getType())) {
					if(!StringUtil.isEmpty(dataGridUrl.getUrlclass())){//倘若urlclass�?为空，则去掉链接�?�?�的"[";
				...
				}															//update-end--Author:liuht  Date:20130228 for：弹出窗�?�设置�?�数�?生效
	...
	/*	sb.append("href+=\"<div class='opts-menu-triangle icon-triangle' title='更多�?作'></div>\";");*/
	...
	protected String formatUrl(String url) {
		...
		String parurlvalue = "";
		if (url.indexOf("&") >= 0) {
			String beforeurl = url.substring(0, url.indexOf("&"));// 截�?�请求地�?�
			String parurl = url.substring(url.indexOf("&") + 1, url.length());// 截�?��?�数
	...
	protected void getField(StringBuffer sb,int frozen) {
		...
		for (DataGridColumn column : columnList) {
			...
			if(StringUtils.isNotEmpty(column.getExtendParams())){
				...
				StringBuffer comboboxStr =new StringBuffer();//声明一个替�?�扩展�?�数中的editor:'combobox'的�?��?
				if(column.getExtendParams().indexOf("editor:'combobox'")>-1){//倘若扩展�?�数中包�?�editor:'combobox'
					if(!StringUtil.isEmpty(column.getDictionary())){//根�?�数�?�字典生�?editor:'combobox'
						if(column.getDictionary().contains(",")){

							if(column.isPopup()){
								//TODO popup值转�?�处�?�，此处暂�?处�?�
							}else{
					...
					column.setExtendParams(column.getExtendParams().replaceAll("editor:'combobox'", comboboxStr.toString()));//替�?�扩展�?�数
			...
			if(column.getFormatterjs()!=null){
				...
				if(StringUtils.isNotEmpty(column.getExtendParams())&&column.getExtendParams().indexOf("editor:'combobox'")>-1){//倘若扩展�?�数中包�?�editor:'combobox'
						//�?�?�?�?增加formatter�?�数，
				}else{
...
//	                                    + " onMouseOver=\"toolTip('<img src='+value+'>')\" onMouseOut=\"toolTip()\" "
				...
				}else if (StringUtil.isNotEmpty(column.getField()) && column.getField().equals("opt")) {// 加入�?作
				...
				else if (columnValueList.size() > 0 && StringUtil.isNotEmpty(column.getField()) && !column.getField().equals("opt")) {// 值替�?�
...
//		sb.append("<link rel=\"stylesheet\" href=\"plug-in/easyui/themes/metro/main.css\" />");
		...
		}else if("single".equals(getQueryMode())&& hasQueryColum(columnList)){//如果表�?�是�?�查询
			sb.append("<span style=\"float:right\">");
			sb.append("<input id=\""+name+"searchbox\" class=\"easyui-searchbox\"  data-options=\"searcher:"+name+ StringUtil.replaceAll("searchbox,prompt:\'{0}\',menu:\'#", "{0}", MutiLangUtil.getLang("common.please.input.keyword")) +name+"mm\'\"></input>");
			sb.append("<div id=\""+name+"mm\" style=\"width:120px\">");
			for (DataGridColumn col : columnList) {
	...
	private void addQueryBuilder(StringBuffer sb,String buttonSytle) {
		...
		appendLine(sb,"<div title=\"查询历�?�\" data-options=\"iconCls:'icon-search'\" style=\"padding:0px;\">");
		appendLine(
				sb,
				"	<ul id=\""
						+ name
		...
		appendLine(sb,"			<table id=\""+name+"tg\" class=\"easyui-treegrid\" title=\"查询�?�件编辑\" style=\"width:450px;height:300px;\"");
		...
		appendLine(sb,"			//url: 'sqlbuilder.json',//�?�以预加载�?�件\r\n");
...
appendLine(sb,"					}}\">关系</th>");
		...
		for (int i=0;i<columnList.size();i++){
			...
			if("opt".equals(col.getField()))continue;//忽略�?作虚拟字段
...
//		appendLine(sb,"				{'fieldId':'office_Phone','fieldName':'办公电�?'},");
		fieldArray.append("]");
		...
		appendLine(sb,"	return '<a  onclick=\\'removeIt('+row.id+')\\' >删除</a>';}\">�?作</th>");
		appendLine(sb,"		</tr>");
		appendLine(sb,"	</thead>");
		appendLine(sb,"	</table>");
		appendLine(sb,"</div>");
		...
		if (btnCls != null && !btnCls.equals("easyui")) {
			...
			appendLine(sb,"<button class=\""+defalutCls+"\" onclick=\"javascript:$('#"+name+"_qbwin').window('close')\">&nbsp;<i class=\"fa fa-close\"></i>�?�消</button>");
			...
			appendLine(sb,"<a class=\""+buttonSytle+"\" data-options=\"iconCls:'icon-cancel'\" href=\"javascript:void(0)\" onclick=\"javascript:$('#"+name+"_qbwin').window('close')\">�?�消</a>");
		}
...
//		appendLine(sb,"<script type=\"text/javascript\" src=\"plug-in/jquery-plugs/storage/jquery.storageapi.min.js\" ></script>"); 
	...
	protected boolean filterBtn = false;//按钮过滤模�?是�?�开�?�，列表上若有一个按钮�?�以让其调用xxFilter函数出现过滤行--
	 ...
	 * * 拼装�?作地�?�,新的风格
	 * @param sb
	 * @param noReturn 是�?��?在该方法中返回href 
	 * @param initGroup 是�?�加载的是�?�?�?��?�
	 */
	protected void getOptUrl(StringBuffer sb,boolean noReturn,boolean initGroup) {
		...
		for (DataGridUrl dataGridUrl : list) {
			if(initGroup){
				//若加载的是组�?��?� 但该�?��?�其实�?是组�?��?�则 跳过
				if(!dataGridUrl.isInGroup()){
					continue;
				}
			}else{
				//若加载的�?是组�?��?� 但该�?��?�其实是组�?��?�则 跳过
				if(dataGridUrl.isInGroup()){
					continue;
				}
			}

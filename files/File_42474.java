...
public class WeiXinMicroUtils {
    ...
    public static Map<String, Object> microSubmit(RpMicroSubmitRecord submitParam) {
        ...
        while (it.hasNext()) {
            ...
            JsonObject o = ele.getAsJsonObject();//数组里�?�的�?一个元素都是一个对象，所以用getAsObject
            serialNo = o.get("serial_no").getAsString();
            JsonObject infoEle = o.get("encrypt_certificate").getAsJsonObject();//你�?的是对象里�?�的info属性，info的值是一个object
            associatedData = infoEle.get("associated_data").getAsString();//从获�?�到的info对象获�?�name的值
            nonce = infoEle.get("nonce").getAsString();//�?��?�
            ciphertextEncrypt = infoEle.get("ciphertext").getAsString();//�?��?�
        }
        try {
            // 获�?��?书明文，备用字段1:APIv3Secret
            ...
            paramMap.put("business_code", submitParam.getBusinessCode());// 业务申请编�?�，订�?��?�，查询状�?时需�?
            ...
            paramMap.put("merchant_shortname", submitParam.getMerchantShortname());//将在支付完�?页�?�买家展示
            paramMap.put("service_phone", submitParam.getServicePhone());//将在交易记录中�?�买家展示，请确�?电�?畅通以便微信回拨确认

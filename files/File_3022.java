...
//                System.out.println("size of LA features in memory:" + effectiveLaSize + "/" + laSize + "=" + format.format(laRatio) + "%");
     ...
     *
     * @param instance        实例
     * @param partialTreeIter �?�标注树的训练迭代数
     * @param i               当�?迭代数
     * @param dataCount
     * @param pool
     * @throws Exception
...
//                System.err.println("...no oracle(" + dataCount + ")...");
            ...
            else
            {
                ...
                {
//                    if (repBeam.size() >= options.beamWidth) // �?��?beamWidth个configuration（这�?�是多余的）
//                        break;
                ...
                if (beam.size() > 0 && oracles.size() > 0)
                {
                    ...
                    {
                        if (options.useRandomOracleSelection) // �?机选择一个 oracle
        ...
        // updating weights
        if (!oracleInBeam ||
                !bestScoringOracle.equals(beam.get(0)) // 虽然oracle在beam里�?�，但在最�?�时刻，它的得分�?是最高
                )
        {
     ...
     *
     * @param instance   训练实例
     * @param oracles    当�?的oracle
     * @param newOracles 储存新oracle
     * @return 这些 oracles 中在模型看�?�分数最大的那个
     * @throws Exception
     */

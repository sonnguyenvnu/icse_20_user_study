...
/**
 * 整个路由框架,整体都是在主线程中执行的,在拦截器中�??供了 callback 机制
 * 所以有耗时的�?作�?�以在拦截器中去开�?线程执行然�?�在回调中继续下一个拦截器
 * <p>
 * 这个类必须放在 {@link ComponentUtil#IMPL_OUTPUT_PKG} 包下�?�
 * 这个类作为框架对外的一个使用的类,里�?�会很多易用的方法
 * <p>
 * time   : 2018/07/26
 *
...
public class Router {
    ...
    /**
     * 空实现,里头都是�?能调用的方法
     * 这个对象�?�会在构建 {@link RouterRequest} 对象失败或者构建之�?就�?�生错误的情况�?会被返回

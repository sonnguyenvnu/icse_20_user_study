...
public class MDAG implements ICacheAble
{
    ...
    /**
     * 简化�?�的根节点（简化指的是用数组简化，而�?是最�?化，这个结构永远是最�?化的）
     */
    ...
    public MDAG(BufferedReader dataFileBufferedReader) throws IOException
    {
        ...
        while ((currentString = dataFileBufferedReader.readLine()) != null)
        {
            ...
            if (mpsIndex != -1)
            {
                String transitionSubstring = previousString.substring(0, mpsIndex);             // 公共�?缀
                String minimizationProcessingSubstring = previousString.substring(mpsIndex);    // �?�?��?�缀
    ...
    /**
     * 计算最�?化的执行�?置，其实是prevStr和currStr的第一个分�?�点<br>
    ...
    {
        int pivotIndex = onePastLastCreatedTransitionSetIndex;  // node自己的�?置
        node.setTransitionSetBeginIndex(pivotIndex);

        onePastLastCreatedTransitionSetIndex += node.getOutgoingTransitionCount();  // 这个�?�数代表id的消耗

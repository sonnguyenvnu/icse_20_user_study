...
public class RedisDeployCenterImpl implements RedisDeployCenter {
    ...
    public RedisOperateEnum addSlotsFailMaster(final long appId, int lossSlotsInstanceId,final String newMasterHost) throws Exception {
        ...
        // 2.获�?�集群中一个�?�康的master作为clusterInfo Nodes的数�?��?
        ...
        // 4.开�?�新的节点
        // 4.1 从newMasterHost找到�?�用的端�?�newMasterPort
        ...
        // 4.2 按照sourceMasterInstance的内存�?�动
        ...
        // 休�?�一段时间，�?�步clusterNodes信�?�
        TimeUnit.SECONDS.sleep(2);
        
        // 8.最终打�?�出当�?还没有补充的slots
    ...
    /**
     * 1. 被forget的节点必须在线(这个�?�件有待验�?) 
     * 2. 被forget的节点�?能有从节点 
     * 3. 被forget的节点�?能有slots
     */
    @Override
    public ClusterOperateResult checkClusterForget(Long appId, int forgetInstanceId) {
        ...
        if (hasSlaves == null || hasSlaves) {
            ...
            return ClusterOperateResult.fail(String.format("被forget的节点(%s:%s)�?能有从节点", forgetHost, forgetPort));
        }
        ...
        if (instanceSlotModel != null && instanceSlotModel.getSlotList() != null
            ...
            return ClusterOperateResult.fail(String.format("被forget的节点(%s:%s)�?能�?有slot", forgetHost, forgetPort));
        }

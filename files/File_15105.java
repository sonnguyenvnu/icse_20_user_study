 ...
 * @see #getListAsync
 * @see #onRefresh
 * @use extends BaseListFragment 并在�?类onCreate中调用onRefresh(...), 具体�?�考.DemoListFragment
 * *缓存使用：在initData�?调用initCache(...), 具体�?�考 .UserListFragment(onCreate方法内)
 */
public abstract class BaseListFragment<T, LV extends AbsListView, BA extends BaseAdapter> extends BaseFragment {
	 ...
	 * @param container
	 * @param savedInstanceState
	 * @param layoutResID fragment全局视图view的布局资�?id，默认值为R.layout.base_http_list_fragment
	 * @return
	 * @must 1.�?�?在�?类�?�?这个类中onCreateView中的代�?;

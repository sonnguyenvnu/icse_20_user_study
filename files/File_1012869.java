...
public class TaskExecute extends CommonService implements Execute<Boolean> {
    ...
    private Boolean runTask(JobMTask onlineTask) {
        ...
        try {
            ...
            LOGGER.error(Constants.LOG_EX_PREFIX + "Task 执行进行�??交线程失败 : jobGroup is {},jobKey is {} taskKey is {}", jobGroup, jobKey, taskKey);
            LOGGER.error(Constants.LOG_EX_PREFIX + "Task 执行异常 :", e);
            isExceptionCountDown(onlineTask, true, "Task 执行进行�??交线程失败" + e.getMessage());
        }
        return Boolean.FALSE;
    }

    /**
     * 判断是�?�是末节点任务以�?��?置任务是�?�都已完�?
     *
     * @param onlineTask
     * @return
     */

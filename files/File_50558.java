...
/**
 * 在调用{@link com.coorchice.library.SuperTextView#setUrlImage(String)}�?�，
 * 如果没有通过{@link ImageEngine#install(Engine)}�?置过图片下载引擎，将使用这个简易版的
 * 图片下载引擎。
 * 建议开�?�者根�?�项目情况自行实现{@link Engine}，然�?�通过{@link ImageEngine#install(Engine)}�?置图片下载引擎。
 */
public class DefaultEngine implements Engine {
  ...
  private static byte[] getBytesArrayFromNet(String path) throws Exception {
    ...
    // 4.将请求�??交到�?务器并得到�?务器端的�?应结果
    int responseCode = httpURLConnection.getResponseCode();// 得到了�?应�?,�?�有�?应�?等于200表示OK
    if (responseCode == 200) {

...
public class CrmLeadsService {
    ...
    public List<Record> information(Integer leadsId) {
        ...
        field.set("线索�??称", crmLeads.getLeadsName()).set("电�?", crmLeads.getMobile())
                .set("手机", crmLeads.getTelephone()).set("下次�?�系时间", DateUtil.formatDateTime(crmLeads.getNextTime()))
                .set("地�?�", crmLeads.getAddress()).set("备注", crmLeads.getRemark());
        ...
        for (String leadsId : leadsIdsArr) {
            ...
            customerFields.forEach(customerField ->{
                leadsFields.forEach(leadsFIeld ->{
                    if ("客户�?��?".equals(customerField.getName()) && "线索�?��?".equals(leadsFIeld.getStr("name"))){
    ...
    public List<Record> queryField() {
        ...
        fieldUtil.getFixedField(fieldList, "leadsName", "线索�??称", "", "text", settingArr, 1);
...
//        fieldUtil.getFixedField(fieldList, "leadsName", "线索�??称", leads.getStr("leads_name"), "text", settingArr, 1);
//        fieldUtil.getFixedField(fieldList, "telephone", "电�?", leads.getStr("telephone"), "text", settingArr, 0);
//        fieldUtil.getFixedField(fieldList, "mobile", "手机", leads.getStr("mobile"), "mobile", settingArr, 0);
//        fieldUtil.getFixedField(fieldList, "address", "地�?�", leads.getStr("address"), "text", settingArr, 0);
//        fieldUtil.getFixedField(fieldList, "nextTime", "下次�?�系时间", DateUtil.formatDateTime(leads.get("next_time")), "datetime", settingArr, 0);
        ...
        try {
            ...
            if (read.size() > 1) {
                ...
                for (int i = 1; i < read.size(); i++) {
                    ...
                    String leadsName = leadsList.get(kv.getInt("线索�??称(*)")).toString();
                    ...
                    if (0 == number) {
                        object.fluentPut("entity", new JSONObject().fluentPut("leads_name", leadsName)
                                .fluentPut("telephone", leadsList.get(kv.getInt("电�?")))
                                .fluentPut("mobile", leadsList.get(kv.getInt("手机")))
                                .fluentPut("address", leadsList.get(kv.getInt("地�?�")))
                                .fluentPut("next_time", leadsList.get(kv.getInt("下次�?�系时间")))

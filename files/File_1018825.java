...
/**
 * 把�?�件classloader注入到parent classloader中，这样�?�以使parent classloader能够找到�?�件中的类。
 * <p>
 * 这�?方案也有�?好的地方，安全性等。
 * <p>
 * 但是 �?�件中通过Intent .put serialize extra 无法找到对应的类。�?�能通过此方法。
 */
public class ClassLoaderInjectHelper {
    ...
    private static InjectResult injectAboveApiLevel22(ClassLoader parentClassLoader, ClassLoader childClassLoader) {
        ...
        try {
            ...
            // 注入 native lib so 目录，需�?parent class loader
            // �??历此目录能够找到。因为注入了以�?�，�?处�?�这个目录找�?到。
    ...
    /**
     * delete elements from specific array object. �?�删除一次，因为 add的时候会添加�?�?的，比如
     * /system/lib 这个删除的时候�?能全部删除
     *
        ...
        for (int i = 0; i < srcLen; i++) {
            ...
            for (int j = 0; j < targetLen; j++) {
                ...
                if (target != null && src.equals(target)) {
                    finded = true;
                    targetCopy.set(j, null); // 找到�?�设置为空，表示已�?从src删除过一次了，
                    break;
                }
            }

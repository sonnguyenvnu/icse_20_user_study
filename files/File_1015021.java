...
public abstract class NormalMessageContentViewHolder extends MessageContentViewHolder {
    ...
    @MessageContextMenuItem(tag = MessageContextMenuItemTags.TAG_RECALL, title = "æ’¤å›ž", priority = 10)
    public void recall(View itemView, UiMessage message) {
        conversationViewModel.recallMessage(message.message);
    }

    @MessageContextMenuItem(tag = MessageContextMenuItemTags.TAG_DELETE, title = "åˆ é™¤", confirm = true, confirmPrompt = "ç¡®è®¤åˆ é™¤æ­¤æ¶ˆæ?¯", priority = 11)
    public void removeMessage(View itemView, UiMessage message) {
        conversationViewModel.deleteMessage(message.message);
    }

    @MessageContextMenuItem(tag = MessageContextMenuItemTags.TAG_FORWARD, title = "è½¬å?‘", priority = 11)
    ...
    @MessageContextMenuItem(tag = MessageContextMenuItemTags.TAG_CHANEL_PRIVATE_CHAT, title = "ç§?è?Š", priority = 12)

...
public class WhereParser {
    ...
    //zhongshu-comment isCondition的�?�?�?�？判断是�?是一个判断�?�件，例如：a=1 或者 floor(a)=1这�?最�?的�?�元
    ...
    private void routeCond(SQLBinaryOpExpr bExpr, SQLExpr sub, Where where) throws SqlParseException {
        if (sub instanceof SQLBinaryOpExpr && (!isCond((SQLBinaryOpExpr) sub) ||
            ...
            if (binarySub.getOperator().priority != bExpr.getOperator().priority) {
                ...
                parseWhere(binarySub, subWhere);//zhongshu-comment 递归调用parseWhere()，解�?出where�?�?�中的多个�?�件
            } else {
                parseWhere(binarySub, where);//zhongshu-comment 递归调用parseWhere()，解�?出where�?�?�中的多个�?�件
            }
        } else if (sub instanceof SQLNotExpr) {
            Where subWhere = new Where(bExpr.getOperator().name);
            where.addWhere(subWhere);
            parseWhere(((SQLNotExpr) sub).getExpr(), subWhere);//zhongshu-comment 递归调用parseWhere()，解�?出where�?�?�中的多个�?�件
    ...
    private void explanCond(String opear, SQLExpr expr, Where where) throws SqlParseException {
        if (expr instanceof SQLBinaryOpExpr) {
        ...
        } else if (expr instanceof SQLInListExpr) { //zhongshu-comment 解�?in和not in语�?�
            ...
            if (isNested)
                ...
                //zhongshu-comment todo 增加代�?

                condition = new Condition(Where.CONN.valueOf(opear),
                        leftSide, //zhongshu-comment 这个�?�数传过去也没有，�?�是SQLCaseExpr对象的地�?�
                        siExpr.getExpr(), //zhongshu-comment 这个�?是有用的�?�数，是SQLCaseExpr对象
            ...
            if (SpatialParamsFactory.isAllowedMethod(methodName)) {
                ...
                /*
                zhongshu-comment 这里也是Script Query，但是貌似没�?过有走这个分支的sql
                1�?文档
    ...
    /**
     * zhongshu-comment 该放方法�?�用于解�?in�?not in括�?�中的列表，将括�?�中的多个值转为数组Object[]
     * @param targetList
     * @return
    ...
    private Object parseValue(SQLExpr expr) throws SqlParseException {
        if (expr instanceof SQLNumericLiteralExpr) {
            ...
            /*
            zhongshu-comment 解�?where�?查询时会抛出这样的异常：

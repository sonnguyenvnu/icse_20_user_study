    ...
    protected <T> Consumer<T> createTaskEventListener(ListenerConfig listenerConfig) {
        ...
        if (null != engine) {
            ...
            return event -> {
                ...
                if (!result.isSuccess()) {
                    throw new BusinessException("æ‰§è¡Œç›‘å?¬å™¨å¤±è´¥:" + result.getMessage(), result.getException());
                }
            };
        } else {
            log.warn("ä¸?æ”¯æŒ?çš„è„šæœ¬è¯­è¨€:{}", listenerConfig.getLanguage());
        }
        return null;
    }
        ...
        return new ProcessConfiguration() {
            ...
            public void assertCanStartProcess(String userId, ProcessDefinition definition) {
                if (!canStartProcess(userId, definition)) {
                    throw new AccessDenyException("æ²¡æœ‰æ?ƒé™?å?¯åŠ¨æ­¤æµ?ç¨‹");
                }
            }

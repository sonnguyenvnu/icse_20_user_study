...
public class BubbleImageView extends ImageView {
    ...
    private boolean mShowText = true;//是�?�显示文字
    private boolean mShowShadow = true;//是�?�显示阴影
    ...
    private void drawText(Canvas canvas, int radiusPx) {
        ...
        if (mShowShadow) {//根�?�是�?��?画阴影
            // 画阴影部分
            mPaint.setColor(Color.parseColor("#70000000"));// �?��?明
            Rect shadowRect = null;
            if (mArrowLocation == LOCATION_LEFT) {
                //如果是在左边
                shadowRect = new Rect(mArrowWidth, 0, getWidth(), getHeight() - getHeight()
                        * percent / 100);//阴影的宽度（图片的宽度）为ImageView的宽度�?去箭头的宽度
            } else {
        ...
        if (mShowText) {//是�?�画文字
            ...
            Rect rect = null;
            int marginLeft = 0;//文字的左边�?
            if (mArrowLocation == LOCATION_LEFT) {//如果是�?�左的

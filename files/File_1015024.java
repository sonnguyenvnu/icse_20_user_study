...
public class VoipMessageViewHolder extends NormalMessageContentViewHolder {
    ...
    public void onBind(UiMessage message) {
        ...
        if (content.getStatus() == 0) {
            ...
            textView.setText("通�?中");
        } else {
            String text;
            if (content.getConnectTime() > 0) {
                long duration = (content.getEndTime() - content.getConnectTime()) / 1000;
                if (duration > 3600) {
                    text = String.format("通�?时长 %d:%02d:%02d", duration / 3600, (duration % 3600) / 60, (duration % 60));
                } else {
                    text = String.format("通�?时长 %02d:%02d", duration / 60, (duration % 60));
                }
            } else {
                text = "对方未接�?�";
            }

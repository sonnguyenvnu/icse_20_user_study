...
/**用户列表界�?�fragment
 * @author Lemon
 * @use new UserListFragment(),详细使用�?.DemoFragmentActivity(initData方法内)
...
//			if (searchType == EditTextInfoWindow.TYPE_NAME) {
				toActivity(EditTextInfoWindow.createIntent(context
						, EditTextInfoWindow.TYPE_NAME, "姓�??", null),
						REQUEST_TO_EDIT_TEXT_INFO_SEARCH, false);
//			} else {
//				toActivity(EditTextInfoWindow.createIntent(context
//						, EditTextInfoWindow.TYPE_PHONE, "手机�?�", null),
		...
		case REQUEST_TO_EDIT_TEXT_INFO_ADD:
			if (data != null) {
				...
				intent = isSearch
						...
						toActivity(intent.putExtra(INTENT_TITLE, "�?�索" + split + value));
			}
			break;
		default:
			break;

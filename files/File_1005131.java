...
@Api(value="JformOrderTicket2",description="è®¢å?•æœºç¥¨ä¿¡æ?¯",tags="jformOrderTicket2Controller")
public class JformOrderTicket2Controller extends BaseController {
	...
	public AjaxJson doDel(JformOrderTicket2Entity jformOrderTicket2, HttpServletRequest request) {
		...
		message = "è®¢å?•æœºç¥¨ä¿¡æ?¯åˆ é™¤æˆ?åŠŸ";
		try{
	...
	public AjaxJson doAdd(JformOrderTicket2Entity jformOrderTicket2, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "è®¢å?•æœºç¥¨ä¿¡æ?¯æ·»åŠ æˆ?åŠŸ";
		try{
			jformOrderMain2Service.save(jformOrderTicket2);
			systemService.addLog(message, Globals.Log_Type_INSERT, Globals.Log_Leavel_INFO);
		}catch(Exception e){
			e.printStackTrace();
			message = "è®¢å?•æœºç¥¨ä¿¡æ?¯æ·»åŠ å¤±è´¥";
			throw new BusinessException(e.getMessage());
		}
		j.setMsg(message);
		return j;
	...
	public AjaxJson doUpdate(JformOrderTicket2Entity jformOrderTicket2, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "è®¢å?•æœºç¥¨ä¿¡æ?¯æ›´æ–°æˆ?åŠŸ";
	...
	public String exportXls(JformOrderTicket2Entity jformOrderTicket2,HttpServletRequest request,HttpServletResponse response
		...
		modelMap.put(NormalExcelConstants.FILE_NAME,"è®¢å?•æœºç¥¨ä¿¡æ?¯");
		modelMap.put(NormalExcelConstants.CLASS,JformOrderTicket2Entity.class);
		modelMap.put(NormalExcelConstants.PARAMS,new ExportParams("è®¢å?•æœºç¥¨ä¿¡æ?¯åˆ—è¡¨", "å¯¼å‡ºäºº:"+ResourceUtil.getSessionUser().getRealName(),
			"å¯¼å‡ºä¿¡æ?¯"));
	...
	@ResponseBody
	@ApiOperation(value="è®¢å?•æœºç¥¨ä¿¡æ?¯åˆ—è¡¨ä¿¡æ?¯",produces="application/json",httpMethod="GET")
	...
	@ResponseBody
	@ApiOperation(value="æ ¹æ?®IDèŽ·å?–è®¢å?•æœºç¥¨ä¿¡æ?¯ä¿¡æ?¯",notes="æ ¹æ?®IDèŽ·å?–è®¢å?•æœºç¥¨ä¿¡æ?¯ä¿¡æ?¯",httpMethod="GET",produces="application/json")
	public ResponseMessage<?> get(@ApiParam(required=true,name="id",value="ID")@PathVariable("id") String id) {
		JformOrderTicket2Entity task = jformOrderMain2Service.get(JformOrderTicket2Entity.class, id);
		if (task == null) {
			return Result.error("æ ¹æ?®IDèŽ·å?–è®¢å?•æœºç¥¨ä¿¡æ?¯ä¿¡æ?¯ä¸ºç©º");
		}
	...
	@ResponseBody
	@ApiOperation(value="åˆ›å»ºè®¢å?•æœºç¥¨ä¿¡æ?¯")
	public ResponseMessage<?> create(@ApiParam(name="è®¢å?•æœºç¥¨ä¿¡æ?¯å¯¹è±¡")@RequestBody JformOrderTicket2Entity jformOrderTicket2, UriComponentsBuilder uriBuilder) {
		...
		try{
			...
			e.printStackTrace();
			return Result.error("è®¢å?•æœºç¥¨ä¿¡æ?¯ä¿¡æ?¯ä¿?å­˜å¤±è´¥");
		}
	...
	@ResponseBody
	@ApiOperation(value="æ›´æ–°è®¢å?•æœºç¥¨ä¿¡æ?¯",notes="æ›´æ–°è®¢å?•æœºç¥¨ä¿¡æ?¯")
	public ResponseMessage<?> update(@ApiParam(name="è®¢å?•æœºç¥¨ä¿¡æ?¯å¯¹è±¡")@RequestBody JformOrderTicket2Entity jformOrderTicket2) {
		//è°ƒç”¨JSR303 Bean Validatorè¿›è¡Œæ ¡éªŒï¼Œå¦‚æžœå‡ºé”™è¿”å›žå?«400é”™è¯¯ç ?å?Šjsonæ ¼å¼?çš„é”™è¯¯ä¿¡æ?¯.
		Set<ConstraintViolation<JformOrderTicket2Entity>> failures = validator.validate(jformOrderTicket2);
		if (!failures.isEmpty()) {
			return Result.error(JSONArray.toJSONString(BeanValidators.extractPropertyAndMessage(failures)));
		...
		try{
			...
			e.printStackTrace();
			return Result.error("æ›´æ–°è®¢å?•æœºç¥¨ä¿¡æ?¯ä¿¡æ?¯å¤±è´¥");
		}

		//æŒ‰Restfulçº¦å®šï¼Œè¿”å›ž204çŠ¶æ€?ç ?, æ— å†…å®¹. ä¹Ÿå?¯ä»¥è¿”å›ž200çŠ¶æ€?ç ?.
		return Result.success("æ›´æ–°è®¢å?•æœºç¥¨ä¿¡æ?¯ä¿¡æ?¯æˆ?åŠŸ");
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
	@ResponseStatus(HttpStatus.NO_CONTENT)
	@ApiOperation(value="åˆ é™¤è®¢å?•æœºç¥¨ä¿¡æ?¯")
	public ResponseMessage<?> delete(@ApiParam(name="id",value="ID",required=true)@PathVariable("id") String id) {
		...
		try {
			...
			e.printStackTrace();
			return Result.error("è®¢å?•æœºç¥¨ä¿¡æ?¯åˆ é™¤å¤±è´¥");
		}

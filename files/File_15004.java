 ...
 * @must 所有请求的url�?请求方法(GET, POST等)�?请求�?�数(key-value方�?，必�?key一定�?加，没�??供的key�?�?加，value�?符�?�指定范围)
 *       都�?符�?��?�端给的接�?�文档
 */
public class HttpRequest {
	 ...
	 * @param requestCode
	 * @param listener
	 * @must request最外层有tag，部分请求还�?currentUserId和对应的password
	 */
	...
	/**用POST方法HEAD数�?�，request和response都�?�明文，�?览器看�?到，用于对安全性�?求高的HEAD请求
	 * @param request
	 * @param requestCode
	 * @param listener
	 * @must request最外层有tag，部分请求还�?currentUserId和对应的password
	...
	/**用POST方法GET数�?�，request和response都�?�明文，�?览器看�?到，用于对安全性�?求高的GET请求
	 * @param request
	 * @param requestCode
	 * @param listener
	 * @must request最外层有tag，部分请求还�?currentUserId和对应的password
	...
	//加 _ 表示class�??，�?��?VERIFY�?知�?�是 Verify 还是 verify 这�?冲�?
	...
	public static final String DATE_UP = "date+";//�?� "date ASC"
	public static final String DATE_DOWN = "date-";//�?� "date DESC"
	...
	/**设置密�?
	 * @param type
	 * @param password �?�需�?phone和verify，�?需�?old password
	 * @param phone
	 * @param verify
	 * @param requestCode
	 * @param listener
	...
	public static void getUserList(int range, long id, com.alibaba.fastjson.JSONObject search, List<Long> idList
		...
		if (idList != null) {
			...
			switch (range) {
			case RANGE_ALL://1.首推注册时间长的（也�?�以是级别高的）；2.给男性用户首推女性用户
				userItem.put("id!", currentUser.getId()); //排除当�?用户
			...
			case RANGE_USER_CIRCLE:
				...
				if (list == null) {//�?能放在range == RANGE_USER_CIRCLE里�?�，为null�?会当�?查询�?�件�?
					list = new ArrayList<Long>();
				}
				if (range == RANGE_USER_CIRCLE) {
					list.add(currentUser.getId());
				} else {//问题�?�能在于登录状�?错误
					list.remove(currentUser.getId());//�?��?误添加
				}
	...
	public static void getMoment(long id, int requestCode, OnHttpResponseListener listener) {
		...
		userItem.setQuery(JSONRequest.QUERY_ALL);//�?�时获�?�Table和total
		request.putsAll(userItem.toArray(10, 0, USER_));
		request.put("praiseCount@", "/User[]/total");//获�?�Table的总数total
		//praise >>>>>>>>>>>>>>>>>>
	 ...
	 * @param momentId
	 * @param toCommentId
	 * @param toUserId �?能�?，�?�能�?�一toCommentId下回�?�?�?�的人
	 * @param content
	 * @param requestCode
	 * @param listener
	 */

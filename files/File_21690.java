...
public class AggMaker {
    ...
    public AggregationBuilder makeGroupAgg(Field field) throws SqlParseException {
        ...
        if (field instanceof MethodField && field.getName().equals("script")) {
            ...
            //question 这里为什么�?将这些信�?�加到groupMap中？
     ...
     * zhongshu-comment 根�?�sql中的函数�?�生�?一些agg，例如sql中的count()�?sum()函数，这是agg链中最里边的那个agg了，eg：
    ...
    private ValuesSourceAggregationBuilder makeCountAgg(MethodField field) {
        ...
        /*
        zhongshu-comment count(1) count(0)这�?应该是查�?到东西的，除�?�你的字段�??就�?��?�1�?0这样
            es的count是针对�?个字段�?�count的，�?下�?�的dsl，对os这个字段�?�count
                "aggregations": {
                    "COUNT(os)": {
                      "value_count": {
                        "field": "os"
            ...
            �?�如你是写count(*)，那es-sql就帮你转�?对"_index"字段�?�count，�?一�?�数�?�都会有"_index"字段，该字段存储的是索引的�??字
         */
        ...
        if ("*".equals(fieldName)) {
            ...
            /*
            zhongshu-comment 这个看起�?�有点多此一举：先将"_index"字符串�?装到KVValue中，然�?��?kv.toString()得到"_index"字符串，还�?如直接将"_index"传进去AggregationBuilders.count(field.getAlias()).field("_index");
            其实目的是为了改�?�形�?�MethodField field的params�?�数中的值，由"*"改为"_index"
             */

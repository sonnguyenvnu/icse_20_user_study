...
//          String type = origDescriptors[i].getPropertyType().toString();
  ...
  /**
   * 将Map内的key与Bean中属性相�?�的内容�?制到BEAN中
   * @param bean Object
  ...
  public static void copyMap2Bean_Nobig(Object bean, Map properties) throws
      ...
      while (names.hasNext()) {
          ...
          Object value = properties.get(name);
          // 命�??应该大�?写应该�?感(�?�则�?��?到对象的属性)
          //name = name.toLowerCase();
          try {
        	  if (value == null) {	// �?光Date类型，好多类型在null时会出错
                  continue;	// 如果为null�?用设 (对象如果有特殊�?始值也�?�以�?留？)
              }
              Class clazz = PropertyUtils.getPropertyType(bean, name);
              if (null == clazz) {	// 在bean中这个属性�?存在
                  continue;
              }
              String className = clazz.getName();
              // 临时对策（如果�?处�?�默认的类型转�?�时会出错）

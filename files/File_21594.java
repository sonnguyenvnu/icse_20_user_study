...
/**
 * 一些具有�?�数的一般在 select 函数.或者group by 函数
 *
 * @author ansj
 */
public class FieldMaker {
    public static Field makeField(SQLExpr expr, String alias, String tableAlias) throws SqlParseException {
        if (expr instanceof SQLIdentifierExpr || expr instanceof SQLPropertyExpr || expr instanceof SQLVariantRefExpr) {
            ...
            /*zhongshu-comment group by�?�?�中case when是没有别�??的，这时alias=null，调用KVValue的toString()会报空指针
            methodParameters.add(new KVValue(alias)); //zhongshu-comment 这是原语�?�，被我注释掉了，改为下�?�带�?�空判断的语�?�*/
    ...
    private static Field handleIdentifier(SQLExpr expr, String alias, String tableAlias) throws SqlParseException {
        ...
        //zhongshu-comment 字段的别�??�?为空 && 别�??和字段�??�?一样
        if (alias != null && !alias.equals(name) && !Util.isFromJoinOrUnionTable(expr)) {

            /*
            zhongshu-comment newFieldName是字段原�?�的�??字，这�?��?应该是用于es dsl的
            使用别�??有很多�?情况：
                1�?最简�?�的就是select field_1 as a from tbl
                2�?调用函数处�?�字段之�?�，select floor(field_1) as a from tbl
                3�?执行表达�?，select field_1 + field_2 as a from tbl
                4�?case when field_1='a' then 'haha' else 'hehe' end as a
                5�?........
            所以这个if分支就是为了处�?�以上这些情况的
             */
            List<SQLExpr> paramers = Lists.newArrayList();
            paramers.add(new SQLCharExpr(alias)); //zhongshu-comment 别�??
    ...
    public static MethodField makeMethodField(String name, List<SQLExpr> arguments, SQLAggregateOption option, String alias, String tableAlias, boolean first) throws SqlParseException {
        ...
        //zhongshu-comment script字段�?会走这个分支

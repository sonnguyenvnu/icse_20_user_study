 ...
 * 		在使用TimeRefresher的Context被销�?�?TimeRefresher.getInstance().removeTimeRefreshListener(...);
 *      在应用退出�?TimeRefresher.getInstance().finish();
 */
public class TimeRefresher {
	...
	static class TimeHolder {
		...
		public void startTimer() {
			...
			timer.schedule(task, 0, duration); //延时0ms�?�执行，1000ms执行一次
			//		Log.w(TAG, "startTimer >>>>>>> " + TimeUtil.getTime(System.currentTimeMillis()));
		}

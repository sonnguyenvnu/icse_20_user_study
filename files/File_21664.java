...
public class ESActionFactory {
	...
	public static QueryAction create(Client client, String sql) throws SqlParseException, SQLFeatureNotSupportedException {
        ...
        switch (firstWord.toUpperCase()) {
			case "SELECT":
			    //zhongshu-comment 将sql字符串解�?�?AST，�?�SQLQueryExpr sqlExpr就是AST了，下�?�的代�?就开始访问AST�?从中获�?�token
				SQLQueryExpr sqlExpr = (SQLQueryExpr) toSqlExpr(sql);
                if(isMulti(sqlExpr)){//zhongshu-comment 判断是�?是union查询，union查询两个select语�?�，btw：�?查询也有多个select语�?�，至少2个
                    ...
                    //todo 看�?懂，测试了好几个常�?的sql，都没有进去handleSubQueries该方法，那就先�?�?�了，看别的
    ...
    private static SQLExpr toSqlExpr(String sql) {
        SQLExprParser parser = new ElasticSqlExprParser(sql); //zhongshu-comment 这个SQLExprParser parser应该就是语法解�?器
        SQLExpr expr = parser.expr(); //zhongshu-comment 这个expr应该就是解�?sql之�?�得到的AST了

        //zhongshu-comment 调用parser.expr()方法解�?完sql语�?��?�，�?�现最�?�一个token�?是End Of File的�?，�?�该sql语�?�貌似是残缺的，�?�能是用户输入了一个未结�?�的sql

...
public class SqlParser {
    ...
    /**
     * zhongshu-comment 在访问AST里�?�的�?�?��?token
     * @param query
     * @return
    ...
    public Select parseSelect(MySqlSelectQueryBlock query) throws SqlParseException {
        ...
        /*zhongshu-comment SqlParser类没有�?员�?��?，里�?�全是方法，所以将this传到WhereParser对象时是无状�?的，
                          �?�SqlParser对象并没有给WhereParser传递任何属性，也�?存在WhereParser修改SqlParser的�?员�?��?值这一说
                         WhereParser�?�是�?�纯想调用SqlParser的方法而已
        */
        ...
        那findSelect()就是解�?这一部分了：a,sum(b),case when c='a' then 1 else 2 end as my_c
         */
        ...
        //zhongshu-comment 这个应该是针对where�?查询的，而�?是from�?查询，貌似�?��?是解�?from�?查询的，报错了
        //zhongshu-comment 也许es本身就�?支�?�?查询，所以es-sql就没实现，那这个fillSubQueries是什么啊？？
        //todo 看�?懂，测试了好几个常�?的sql，都没有进去该方法，那就先�?�?�了，看别的
        select.fillSubQueries();

        //zhongshu-comment 解�?sql语�?�中的注释：select /*! USE_SCROLL(10,120000) */ * FROM spark_es_table
        //hint�?��?的�?�?是�??示，�?�sql中的注释内容
        // /* 和 */之间是sql的注释内容，这是sql本身的语法，然�?�sql解�?器会将注释�?�之间的内容“! USE_SCROLL(10,120000) �?抽�?�出�?�
        // ! USE_SCROLL是es-sql自己定义的一套规则，
        // 在�?增加mysql原有语法的情况下，利用注释�?��?�活地扩展es-sql的功能，这样就能使用druid的mysql语法解�?器了，无需自己实现
        // 注�?：!�?��?�和USE_SCROLL之间�?空且�?�能空一格
    ...
    private void findGroupBy(MySqlSelectQueryBlock query, Select select) throws SqlParseException {
        ...
        for (SQLExpr sqlExpr : items) {
            ...
            if ((sqlExpr instanceof SQLParensIdentifierExpr || !(sqlExpr instanceof SQLIdentifierExpr || sqlExpr instanceof SQLMethodInvokeExpr)) && !standardGroupBys.isEmpty()) {
                ...
                // zhongshu-comment 先将standardGroupBys里�?�的字段传到select对象的groupBys字段中，然�?�给standardGroupBys分�?一个没有元素的新的list
    ...
    private List<Field> convertExprsToFields(List<? extends SQLExpr> exprs, SQLTableSource sqlTableSource) throws SqlParseException {
        ...
        for (SQLExpr expr : exprs) {
            ...
            //zhongshu-comment case when 有别�??过�?了语法解�?，没有别�??执行下�?�语�?�会报空指针
    ...
    private void addOrderByToSelect(Select select, List<SQLSelectOrderByItem> items, String alias) throws SqlParseException {
        for (SQLSelectOrderByItem sqlSelectOrderByItem : items) {
            ...
            if (sqlSelectOrderByItem.getType() == null) {
                sqlSelectOrderByItem.setType(SQLOrderingSpecification.ASC); //zhongshu-comment 默认是�?��?排�?
            }
     ...
     * zhongshu-comment �?�解�?了一般查询和join查询，没有解�?�?查询
     * @param from the from clause.
     * @return list of From objects represents all the sources.
     */
    private List<From> findFrom(SQLTableSource from) {
        //zhongshu-comment class1.isAssignableFrom(class2) class2是�?是class1的�?类或者�?接�?�
        //改�?用instanceof 应该也行�?�：from instanceof SQLExprTableSource

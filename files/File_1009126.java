...
public final class MigrationHelper {
    ...
    public static void migrate(SQLiteDatabase db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        printLog("ã€?The Old Database Versionã€‘" + db.getVersion());
    ...
    public static void migrate(Database database, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        printLog("ã€?Generate temp tableã€‘start");
        generateTempTables(database, daoClasses);
        printLog("ã€?Generate temp tableã€‘complete");
        ...
        if (listener != null) {
            ...
            printLog("ã€?Drop all table by listenerã€‘");
            listener.onCreateAllTables(database, false);
            printLog("ã€?Create all table by listenerã€‘");
        } else {
            dropAllTables(database, true, daoClasses);
            createAllTables(database, false, daoClasses);
        }
        printLog("ã€?Restore dataã€‘start");
        restoreData(database, daoClasses);
        printLog("ã€?Restore dataã€‘complete");
    }
    ...
    private static void generateTempTables(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        for (int i = 0; i < daoClasses.length; i++) {
            ...
            if (!isTableExists(db, false, tableName)) {
                printLog("ã€?New Tableã€‘" + tableName);
                continue;
            }
            try {
                ...
                printLog("ã€?Tableã€‘" + tableName +"\n ---Columns-->"+getColumnsStr(daoConfig));
                printLog("ã€?Generate temp tableã€‘" + tempTableName);
            } catch (SQLException e) {
                Log.e(TAG, "ã€?Failed to generate temp tableã€‘" + tempTableName, e);
            }
        }
    }
    ...
    private static void dropAllTables(Database db, boolean ifExists, @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
        ...
        printLog("ã€?Drop all table by reflectã€‘");
    }

    private static void createAllTables(Database db, boolean ifNotExists, @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
        reflectMethod(db, "createTable", ifNotExists, daoClasses);
        printLog("ã€?Create all table by reflectã€‘");
    }
        ...
        for (int i = 0; i < daoClasses.length; i++) {
            ...
            try {
                ...
                if (intoColumns.size() != 0) {
                    ...
                    db.execSQL(insertTableStringBuilder.toString());
                    printLog("ã€?Restore dataã€‘ to " + tableName);
                }
                StringBuilder dropTableStringBuilder = new StringBuilder();
                dropTableStringBuilder.append("DROP TABLE ").append(tempTableName);
                db.execSQL(dropTableStringBuilder.toString());
                printLog("ã€?Drop temp tableã€‘" + tempTableName);
            } catch (SQLException e) {
                Log.e(TAG, "ã€?Failed to restore data from temp table ã€‘" + tempTableName, e);
            }
        }
    }
    ...
    private static class TableInfo {
        ...
        private static List<TableInfo> getTableInfo(Database db, String tableName) {
            ...
            while (cursor.moveToNext()) {
                ...
                // printLog(tableName + "ï¼š" + tableInfo);
            }
            cursor.close();
            return tableInfos;
        }

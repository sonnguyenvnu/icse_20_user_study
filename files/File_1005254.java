...
public class CommonController extends BaseController {
	...
	public ModelAndView listTurn(HttpServletRequest request) {
		String turn = request.getParameter("turn");// 跳转的目标页�?�
		logger.info("--通用页�?�跳转--listTurn-------"+turn);
	...
	public void getAutoList(HttpServletRequest request, HttpServletResponse response, Autocomplete autocomplete) {
		...
		String trem = StringUtil.getEncodePra(request.getParameter("trem"));// �?新解�?�?�数
	...
	public AjaxJson delObjFile(HttpServletRequest request) {
		...
		String subclassname = attachment.getSubclassname(); // �?类类�??
		Object objfile = systemService.getEntity(MyClassLoader.getClassByScn(subclassname), attachment.getId());// �?类对象
		message = "" + attachment.getAttachmenttitle() + "删除�?功";
	...
	public void objfileGrid(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		String subclassname = oConvertUtils.getString(request.getParameter("subclassname"));// �?类类�??
	...
	/**
	 * @param dictionary 自定义字典(格�?：表�??,编�?,显示文本)
	 * @param dictCondition 自定义字典表的显示文本-字典查询�?�件
	 * @param request
	 * @return
	 */

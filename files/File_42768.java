...
public class OperatorRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		if (StringUtils.isEmpty(loginName.trim())) {
			throw new UnknownAccountException();// 没找到�?�?�
		}
		...
		if (PublicStatusEnum.UNACTIVE.equals(operator.getStatus())) {
			throw new LockedAccountException(); // �?�?��?定
		}

		// 交给AuthenticatingRealm使用CredentialsMatcher进行密�?匹�?，如果觉得人家的�?好�?�以自定义实现
		SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(operator.getLoginName(), // 登录�??
				operator.getLoginPwd(), // 密�?

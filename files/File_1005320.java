...
public class UserController extends BaseController {
	...
	public void menu(HttpServletRequest request, HttpServletResponse response) {
		...
		// 登陆者的�?��?
		Set<TSFunction> loginActionlist = new HashSet<TSFunction>();// 已有�?��?�?��?�
		...
		List<TSFunction> bigActionlist = new ArrayList<TSFunction>();// 一级�?��?�?��?�
		List<TSFunction> smailActionlist = new ArrayList<TSFunction>();// 二级�?��?�?��?�
	...
	public AjaxJson delInterfaceUser(@RequestParam(required = true) String userid) {
		...
		try {
			...
			if(user!=null) {
				...
				ajaxJson.setMsg("删除�?功");
			}else {
				ajaxJson.setMsg("用户�?存在");
			}
	...
	public AjaxJson savenewpwd(HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		TSUser user = ResourceUtil.getSessionUser();
		logger.info("["+IpUtil.getIpAddr(request)+"][修改密�?] start");
		...
		if (!pString.equals(user.getPassword())) {
			...
			systemService.updateEntitie(user);
			j.setMsg("修改�?功");
			logger.info("["+IpUtil.getIpAddr(request)+"][修改密�?]修改�?功 userId:"+user.getUserName());
	...
	public ModelAndView changepasswordforuser(TSUser user, HttpServletRequest req) {
		logger.info("["+IpUtil.getIpAddr(req)+"][跳转�?置用户密�?页�?�]["+user.getUserName()+"]");
	...
	public AjaxJson savenewpwdforuser(HttpServletRequest req) {
		logger.info("["+IpUtil.getIpAddr(req)+"][�?置密�?] start");
		String message = null;
		AjaxJson j = new AjaxJson();
		...
		if (StringUtil.isNotEmpty(id)) {
			...
			if("admin".equals(users.getUserName()) && !"admin".equals(ResourceUtil.getSessionUser().getUserName())){
				message = "超级管�?�员[admin]，�?�有admin本人�?��?作，其他人无�?��?!";
			...
			message = "用户: " + users.getUserName() + "密�?�?置�?功";
			systemService.addLog(message, Globals.Log_Type_UPDATE, Globals.Log_Leavel_INFO);
			logger.info("["+IpUtil.getIpAddr(req)+"][�?置密�?]"+message);
		} 
	...
	public AjaxJson lock(String id, HttpServletRequest req) {
		...
		if("0".equals(lockValue)){
			message = "用户：" + user.getUserName() + "�?定�?功!";
		}else if("1".equals(lockValue)){
			message = "用户：" + user.getUserName() + "激活�?功!";
		}
		systemService.addLog(message, Globals.Log_Type_UPDATE, Globals.Log_Leavel_INFO);
		logger.info("["+IpUtil.getIpAddr(req)+"][�?定账户]"+message);
		}catch(Exception e){
			message = "�?作失败!";
		}
		j.setMsg(message);
...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        cq.in("status", userstate);
        cq.eq("deleteFlag", Globals.Delete_Normal);
        cq.eq("userType", Globals.USER_TYPE_SYSTEM);//用户列表�?显示接�?�类型的用户
...
//		List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
		if (!user.getStatus().equals(Globals.User_ADMIN)) {
			...
			userService.updateEntitie(user);
			message = "用户：" + user.getUserName() + "删除�?功";
...
/**
			if (roleUser.size()>0) {
                ...
                systemService.executeSql("delete from t_s_user_org where user_id=?", user.getId()); // 删除 用户-机构 数�?�
...
**/	
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			if (users != null) {
				...
				message = "用户: " + user.getUserName() + "添加�?功";
	...
	public ModelAndView roles(HttpServletRequest request) {
		//--author：zhoujf-----start----date:20150531--------for: 编辑用户，选择角色,弹出的角色列表页�?�，默认没选中
	...
	public void datagridRole(TSRole tsRole, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		cq.eq("roleType", OrgConstants.SYSTEM_ROLE_TYPE);//默认�?�查询系统角色
...
//	}
	...
	public AjaxJson savesign(HttpServletRequest req) {
		...
		AjaxJson j = new AjaxJson();
		message = user.getUserName() + "设置签�??�?功";
...
//			}
			...
			if(StringUtils.isNotEmpty(indexStyle)){
				...
				logger.debug(" ----- 首页样�?: indexStyle ----- "+indexStyle);
				j.setSuccess(Boolean.TRUE);
				j.setMsg("样�?修改�?功，请刷新页�?�");
			}
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				...
				for (TSUser tsUser : tsUsers) {
					...
					tsUser.setUserType(Globals.USER_TYPE_SYSTEM);//导入用户 在用户管�?�列表�?显示

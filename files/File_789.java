...
//    }
    ...
    public static Date castToDate(Object value, String format){
        ...
        if(value instanceof Date){ // 使用频率最高的，应优先处�?�
            return (Date) value;
        }
    ...
    public static java.sql.Date castToSqlDate(Object value){
        ...
        if(longValue <= 0){
            throw new JSONException("can not cast to Date, value : " + value); // TODO 忽略 1970-01-01 之�?的时间处�?�？
        }
    ...
    public static SerializeBeanInfo buildBeanInfo(Class<?> beanType //
        ...
        // fieldName,field ，先生�?fieldName的快照，�?少之�?�的findField的轮询
    ...
    public static List<FieldInfo> computeGetters(Class<?> clazz, //
        ...
        for(Method method : methods){
            ...
            /**
             *  如果在属性或者方法上存在JSONField注解，并且定制了name属性，�?以类上的propertyNamingStrategy设置为准，以此字段的JSONField的name定制为准。
             */
    ...
    private static boolean isJSONTypeIgnore(Class<?> clazz, String propertyName){
        JSONType jsonType = TypeUtils.getAnnotation(clazz,JSONType.class);
        if(jsonType != null){
            // 1�?新增 includes 支�?，如果 JSONType �?�时设置了includes 和 ignores 属性，则以includes为准。
            // 2�?个人认为对于大�?写�?感的Java和JS而言，使用 equals() 比 equalsIgnoreCase() 更好，改动的唯一风险就是�?��?�兼容性的问题
            // �?过，相信开�?�者应该都是严格按照大�?写�?感的方�?进行属性设置的

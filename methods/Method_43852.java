@Override public String changeOrder(LimitOrder limitOrder) throws IOException {
  ...
  if (limitOrder.getType().equals(Order.OrderType.ASK)) {
    response=coinmateReplaceBySellLimit(limitOrder.getId(),limitOrder.getOriginalAmount(),limitOrder.getLimitPrice(),CoinmateUtils.getPair(limitOrder.getCurrencyPair()),null,hidden ? 1 : 0,immediateOrCancel ? 1 : 0,trailing ? 1 : 0);
  }
 else   if (limitOrder.getType().equals(Order.OrderType.BID)) {
    response=coinmateReplaceByBuyLimit(limitOrder.getId(),limitOrder.getOriginalAmount(),limitOrder.getLimitPrice(),CoinmateUtils.getPair(limitOrder.getCurrencyPair()),null,hidden ? 1 : 0,immediateOrCancel ? 1 : 0,trailing ? 1 : 0);
  }
 else {

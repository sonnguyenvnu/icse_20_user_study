public void Bind(){
    ...
    @Override public void onClick(    View v){
      ...
      for (      final Pair<Integer,Integer> pair : ColorPreferences.themePairList) {
        dialoglayout.findViewById(pair.first).setOnClickListener(new View.OnClickListener(){
          @Override public void onClick(          View v){
            SettingsThemeFragment.changed=true;
            String[] names=new ColorPreferences(context).getFontStyle().getTitle().split("_");
  ...
  context.findViewById(R.id.settings_theme_main).setOnClickListener(new View.OnClickListener(){
    @Override public void onClick(    View v){
      ...
      colorPicker2.setOnColorChangedListener(new OnColorChangedListener(){
        @Override public void onColorChanged(        int i){
          SettingsThemeFragment.changed=true;
      ...
      dialoglayout.findViewById(R.id.ok).setOnClickListener(new View.OnClickListener(){
        @Override public void onClick(        View v){
          if (SettingValues.colorIcon) {
            context.getPackageManager().setComponentEnabledSetting(new ComponentName(context,ColorPreferences.getIconName(context,Reddit.colors.getInt("DEFAULTCOLOR",0))),PackageManager.COMPONENT_ENABLED_STATE_DISABLED,PackageManager.DONT_KILL_APP);
            context.getPackageManager().setComponentEnabledSetting(new ComponentName(context,ColorPreferences.getIconName(context,colorPicker2.getColor())),PackageManager.COMPONENT_ENABLED_STATE_ENABLED,PackageManager.DONT_KILL_APP);
          }
  ...
  ((TextView)context.findViewById(R.id.settings_theme_tint_current)).setText(SettingValues.colorBack ? (SettingValues.colorSubName ? context.getString(R.string.subreddit_name_tint) : context.getString(R.string.card_background_tint)) : context.getString(R.string.misc_none));
  boolean enabled=!((TextView)context.findViewById(R.id.settings_theme_tint_current)).getText().equals(context.getString(R.string.misc_none));
...
final SwitchCompat colorNavbarSwitch=(SwitchCompat)context.findViewById(R.id.settings_theme_color_navigation_bar);
...
if (isChecked) {
context.getPackageManager().setComponentEnabledSetting(new ComponentName(context,Slide.class.getPackage().getName() + ".Slide"),PackageManager.COMPONENT_ENABLED_STATE_DISABLED,PackageManager.DONT_KILL_APP);
context.getPackageManager().setComponentEnabledSetting(new ComponentName(context,ColorPreferences.getIconName(context,Reddit.colors.getInt("DEFAULTCOLOR",0))),PackageManager.COMPONENT_ENABLED_STATE_ENABLED,PackageManager.DONT_KILL_APP);
}
 else {
context.getPackageManager().setComponentEnabledSetting(new ComponentName(context,ColorPreferences.getIconName(context,Reddit.colors.getInt("DEFAULTCOLOR",0))),PackageManager.COMPONENT_ENABLED_STATE_DISABLED,PackageManager.DONT_KILL_APP);
context.getPackageManager().setComponentEnabledSetting(new ComponentName(context,Slide.class.getPackage().getName() + ".Slide"),PackageManager.COMPONENT_ENABLED_STATE_ENABLED,PackageManager.DONT_KILL_APP);
}
}
}
);
...
SettingValues.prefs.edit().putInt(SettingValues.PREF_NIGHT_MODE_STATE,position).apply();
SettingsThemeFragment.changed=true;
}
...
SettingValues.prefs.edit().putInt(SettingValues.PREF_NIGHT_THEME,pair.second).apply();
}
}
}
);
...
;
dialoglayout.findViewById(R.id.start_spinner_layout).setVisibility(View.VISIBLE);
final ArrayAdapter<String> startAdapter=new ArrayAdapter<>(context,android.R.layout.simple_spinner_item,timesStart);
...
;
dialoglayout.findViewById(R.id.end_spinner_layout).setVisibility(View.VISIBLE);
final ArrayAdapter<String> endAdapter=new ArrayAdapter<>(context,android.R.layout.simple_spinner_item,timesEnd);
...
new AlertDialogWrapper.Builder(context).setTitle(R.string.general_nighttheme_ispro).setMessage(R.string.pro_upgrade_msg).setPositiveButton(R.string.btn_yes_exclaim,new DialogInterface.OnClickListener(){
public void onClick(DialogInterface dialog,int whichButton){
try {
context.startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse("market://details?id=" + context.getString(R.string.ui_unlock_package))));
}
 catch (android.content.ActivityNotFoundException anfe) {
context.startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse("http://play.google.com/store/apps/details?id=" + context.getString(R.string.ui_unlock_package))));
}
}
}

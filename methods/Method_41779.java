@Override public void start(UserTransaction userTransaction){
  try {
    if (jobFiles.isEmpty() == false) {
      ...
      while (iterator.hasNext()) {
        ...
        if (scanInterval > 0) {
          ...
          JobDetail job=newJob().withIdentity(jobTriggerName,JOB_INITIALIZATION_PLUGIN_NAME).ofType(FileScanJob.class).usingJobData(FileScanJob.FILE_NAME,jobFile.getFileName()).usingJobData(FileScanJob.FILE_SCAN_LISTENER_NAME,JOB_INITIALIZATION_PLUGIN_NAME + '_' + getName()).build();
          SimpleTrigger trig=newTrigger().withIdentity(tKey).withSchedule(simpleSchedule().repeatForever().withIntervalInMilliseconds(scanInterval)).forJob(job).build();
